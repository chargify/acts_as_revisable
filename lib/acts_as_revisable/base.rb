...
module WithoutScope
  ...
  REVISABLE_SYSTEM_COLUMNS = %w(revisable_original_id revisable_branched_from_id revisable_number revisable_name revisable_type revisable_current_at revisable_revised_at revisable_deleted_at revisable_is_current).freeze

  # define the ActiveRecord magic columns that should not be monitored
  REVISABLE_UNREVISABLE_COLUMNS = %w(id type created_at updated_at).freeze
  ...
  module ActsAsRevisable
    ...
    module ClassMethods
      ...
      def acts_as_revisable(*args, &block)
        ...
        send(:include, Revisable)
        send(:include, Deletable) if revisable_options.on_delete == :revise
      end
      ...
      def revisable_shared_setup(args, block)
        ...
        send(:include, Common)
        send(:extend, Validations) unless revisable_options.no_validation_scoping?
      end
    end
  end
end
